package vista;
import controlador.*;
public class VBuscaCuentaPagar extends javax.swing.JFrame {
public  static String sd= "";
 static String Cliente= "";
public  static String Id1= "";
 static String Cliente1= "";
    /**
     * Creates new form VBarrio
     */
    public VBuscaCuentaPagar() {
        initComponents();
    }
    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        jTabbedPane1 = new javax.swing.JTabbedPane();
        jPanel1 = new javax.swing.JPanel();
        jScrollPane1 = new javax.swing.JScrollPane();
        TablaConsulta = new javax.swing.JTable();
        CampoBuscar = new javax.swing.JTextField();
        jLabel2 = new javax.swing.JLabel();
        Seleccionar = new javax.swing.JButton();
        CampoCodigo = new javax.swing.JTextField();
        Camponom = new javax.swing.JTextField();
        contador = new javax.swing.JTextField();
        monto = new javax.swing.JTextField();
        telefono4 = new javax.swing.JTextField();
        jLabel1 = new javax.swing.JLabel();

        setDefaultCloseOperation(javax.swing.WindowConstants.EXIT_ON_CLOSE);
        setMinimumSize(new java.awt.Dimension(1250, 450));
        getContentPane().setLayout(null);

        jTabbedPane1.setBorder(javax.swing.BorderFactory.createCompoundBorder());
        jTabbedPane1.setFont(new java.awt.Font("Purisa", 1, 18)); // NOI18N
        jTabbedPane1.setMinimumSize(new java.awt.Dimension(147, 540));
        jTabbedPane1.setPreferredSize(new java.awt.Dimension(480, 360));

        jPanel1.setBackground(new java.awt.Color(108, 99, 255));
        jPanel1.setLayout(null);

        TablaConsulta.setFont(new java.awt.Font("Purisa", 1, 18)); // NOI18N
        TablaConsulta.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null}
            },
            new String [] {
                "Title 1", "Title 2", "Title 3", "Title 4"
            }
        ));
        TablaConsulta.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                TablaConsultaMouseClicked(evt);
            }
        });
        jScrollPane1.setViewportView(TablaConsulta);

        jPanel1.add(jScrollPane1);
        jScrollPane1.setBounds(10, 50, 1190, 180);

        CampoBuscar.setFont(new java.awt.Font("Purisa", 1, 14)); // NOI18N
        jPanel1.add(CampoBuscar);
        CampoBuscar.setBounds(1010, 10, 190, 40);

        jLabel2.setFont(new java.awt.Font("Purisa", 1, 18)); // NOI18N
        jLabel2.setForeground(new java.awt.Color(255, 255, 255));
        jLabel2.setText("Buscar ");
        jPanel1.add(jLabel2);
        jLabel2.setBounds(910, 10, 100, 40);

        Seleccionar.setFont(new java.awt.Font("Purisa", 1, 18)); // NOI18N
        Seleccionar.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagen2/seleccione.png"))); // NOI18N
        Seleccionar.setText("Seleccionar");
        Seleccionar.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                SeleccionarActionPerformed(evt);
            }
        });
        jPanel1.add(Seleccionar);
        Seleccionar.setBounds(1020, 230, 180, 40);

        CampoCodigo.setEditable(false);
        CampoCodigo.setFont(new java.awt.Font("Purisa", 1, 18)); // NOI18N
        jPanel1.add(CampoCodigo);
        CampoCodigo.setBounds(10, 10, 40, 40);

        Camponom.setEditable(false);
        Camponom.setFont(new java.awt.Font("Purisa", 1, 18)); // NOI18N
        jPanel1.add(Camponom);
        Camponom.setBounds(50, 11, 90, 40);

        contador.setEditable(false);
        contador.setFont(new java.awt.Font("Purisa", 1, 18)); // NOI18N
        jPanel1.add(contador);
        contador.setBounds(400, 10, 130, 40);

        monto.setEditable(false);
        monto.setFont(new java.awt.Font("Purisa", 1, 18)); // NOI18N
        jPanel1.add(monto);
        monto.setBounds(270, 10, 130, 40);

        telefono4.setEditable(false);
        telefono4.setFont(new java.awt.Font("Purisa", 1, 18)); // NOI18N
        jPanel1.add(telefono4);
        telefono4.setBounds(140, 10, 130, 40);

        jTabbedPane1.addTab("Listado", jPanel1);

        getContentPane().add(jTabbedPane1);
        jTabbedPane1.setBounds(0, 50, 1220, 330);

        jLabel1.setFont(new java.awt.Font("Purisa", 1, 18)); // NOI18N
        jLabel1.setText("Cuentas a Pagar");
        getContentPane().add(jLabel1);
        jLabel1.setBounds(510, 20, 210, 30);

        pack();
    }// </editor-fold>//GEN-END:initComponents

    private void SeleccionarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SeleccionarActionPerformed
        sd= this.CampoCodigo.getText();
		Cliente= this.Camponom.getText();
		Id1= this.monto.getText();
       new cFormaPagoCuentaPagar(new vFormaPagoCuentaPagar()).iniciar();
        dispose();
    }//GEN-LAST:event_SeleccionarActionPerformed

    private void TablaConsultaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_TablaConsultaMouseClicked
   
        // TODO add your handling code here:
    }//GEN-LAST:event_TablaConsultaMouseClicked
    /**
     * @param args the command line arguments
     */
    public static void main(String args[]) {
        /*
        Create and display the form */
        
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
             new CBuscaCuentaPagar(new VBuscaCuentaPagar()).iniciar();            }
        });
    }
    // Variables declaration - do not modify//GEN-BEGIN:variables
    public javax.swing.JTextField CampoBuscar;
    public javax.swing.JTextField CampoCodigo;
    public javax.swing.JTextField Camponom;
    private javax.swing.JButton Seleccionar;
    public javax.swing.JTable TablaConsulta;
    public javax.swing.JTextField contador;
    private javax.swing.JLabel jLabel1;
    private javax.swing.JLabel jLabel2;
    private javax.swing.JPanel jPanel1;
    private javax.swing.JScrollPane jScrollPane1;
    public javax.swing.JTabbedPane jTabbedPane1;
    public javax.swing.JTextField monto;
    public javax.swing.JTextField telefono4;
    // End of variables declaration//GEN-END:variables
}