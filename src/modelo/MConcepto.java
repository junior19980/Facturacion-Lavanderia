...
public class MConcepto extends ConexionBD {
...
if( valida_datos(Cargo))
         {
            //Se arma la consulta
            String q = " INSERT INTO Conceptos ( idConcepto , concepto,precio) "
    ...
    public boolean Actualizar(String id, String nombre,String precio) {
        String q = "UPDATE Conceptos SET concepto='" + nombre + "', precio='" + precio + "'  WHERE idConcepto ='" + id + "' ";
    ...
    public boolean Eliminar(String id) {
        boolean res = false;
        //se arma la consulta
        String q = " DELETE FROM Conceptos WHERE idConcepto='" + id + "' ";
        //se ejecuta la consulta
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement(q);
            pstm.execute();
    ...
    public void generar_codigo() {
        try {
            PreparedStatement pstm2 = this.getConnection().prepareStatement("select MAX(idConcepto) + 1 as id from Conceptos");
    ...
    public void Reportes(String archivo) {
        try {
            PreparedStatement psta = this.getConnection().prepareStatement("select * FROM Conceptos order by idConcepto");
