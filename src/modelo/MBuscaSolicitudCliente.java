...
public class MBuscaSolicitudCliente extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
      ...
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        ...
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vsolicitudes WHERE idSolicitud LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vsolicitudes  WHERE idSolicitud LIKE '%" + filtro + "%' ORDER BY idSolicitud");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idSolicitud");
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE solicitudes SET estado ='Anulado' WHERE idSolicitud ='" + idModelo + "' ";
