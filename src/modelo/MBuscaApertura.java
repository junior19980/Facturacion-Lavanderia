...
public class MBuscaApertura extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo",  "Hora Apertura", "Fecha Apertura"};
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vapertura WHERE idAperturaCaja LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vapertura  WHERE idAperturaCaja LIKE '%" + filtro + "%' ORDER BY idAperturaCaja");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE aperturacajacierre SET estado ='Anulado' WHERE idAperturaCaja ='" + idModelo + "' ";
