...
public class MBuscaFacturaEntrega extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo", "Cliente", "Ruc", "idServicio", "Descripcion", "Cantidad"};
        //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vfacturaentrega WHERE nombre_RS LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vfacturaentrega  WHERE nombre_RS LIKE '%" + filtro + "%' ORDER BY idFacturaServicio");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idFacturaServicio");
                data[i][1] = res.getString("nombre_RS");
                data[i][2] = res.getString("ci_RUC");
                data[i][3] = res.getString("idServicio");
                data[i][4] = res.getString("Servicio");
    ...
    public boolean Modificar(String idModelo, String idServicio) {
        String q = "UPDATE detallesfacturasservicios SET Estado ='2' WHERE idFacturaServicio='" + idModelo + "' and   idServicio ='" + idServicio + "' ";
