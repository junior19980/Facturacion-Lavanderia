...
public class MBuscaNotaDebitoCompra extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
      ...
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        ...
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vnotadebitocompra WHERE NroNotaDebitoCompra LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vnotadebitocompra  WHERE NroNotaDebitoCompra LIKE '%" + filtro + "%' ORDER BY NroNotaDebitoCompra");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("NroNotaDebitoCompra");
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE notadebitocompras SET estado ='Anulado' WHERE NroNotaDebitoCompra ='" + idModelo + "' ";
