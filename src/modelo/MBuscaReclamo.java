...
public class MBuscaReclamo extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
      ...
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        ...
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vreclamo WHERE idReclamos LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vreclamo  WHERE idReclamos LIKE '%" + filtro + "%' ORDER BY idReclamos");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idReclamos");
                data[i][1] = res.getString("fecha");
                data[i][2] = res.getString("ObsMotivoReclamo");
                i++;
            }
            res.close();
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE reclamos SET estado ='Anulado' WHERE idReclamos ='" + idModelo + "' ";
