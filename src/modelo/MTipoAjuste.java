...
public class MTipoAjuste extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
      ...
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        ...
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM TiposAjustes  WHERE TipoAjuste LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM TiposAjustes  WHERE TipoAjuste LIKE '%" + filtro + "%' ORDER BY idTipoAjuste");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idTipoAjuste");
                data[i][1] = res.getString("TipoAjuste");
...
if( valida_datos(Cargo))
         {
            //Se arma la consulta
            String q = " INSERT INTO TiposAjustes ( idTipoAjuste , TipoAjuste) "
    ...
    public boolean Actualizar(String id, String nombre) {
        String q = "UPDATE TiposAjustes SET TipoAjuste='" + nombre + "'  WHERE idTipoAjuste ='" + id + "' ";
    ...
    public boolean Eliminar(String id) {
        boolean res = false;
        //se arma la consulta
        String q = " DELETE FROM TiposAjustes WHERE idTipoAjuste='" + id + "' ";
        //se ejecuta la consulta
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement(q);
            pstm.execute();
    ...
    public void generar_codigo() {
        try {
            PreparedStatement pstm2 = this.getConnection().prepareStatement("select MAX(idTipoAjuste) + 1 as id from TiposAjustes");
