...
public class MBuscaPresupuesto1 extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo", "Fecha", "Cliente", "CI_RUC", "Monto"};
        //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vPresupuestosMontos WHERE nombre_RS LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vPresupuestosMontos  WHERE nombre_RS LIKE '%" + filtro + "%' ORDER BY idPresupuestoServicio");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idPresupuestoServicio");
                data[i][1] = res.getString("fecha");
                data[i][2] = res.getString("nombre_RS");
                data[i][3] = res.getString("CI_RUC");
                data[i][4] = res.getString("Total");
                i++;
            }
            res.close();
   ...
   /* public boolean Modificar(String idModelo) {
        String q = "UPDATE vPresupuestosMontos SET idEstado =2 WHERE idPresupuesto ='" + idModelo + "' ";
