...
public class MBuscaArqueo extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo Arqueo",  "Codigo Apertura", "Monto Arqueo"};
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM arqueos WHERE idArqueoCaja LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM arqueos  WHERE idArqueoCaja LIKE '%" + filtro + "%' ORDER BY idArqueoCaja");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idArqueoCaja");
                data[i][1] = res.getString("idAperturaCaja");
                data[i][2] = res.getString("MontoArqueo");
                i++;
            }
            res.close();
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE arqueocajas SET estado ='Anulado' WHERE idArqueoCaja ='" + idModelo + "' ";
