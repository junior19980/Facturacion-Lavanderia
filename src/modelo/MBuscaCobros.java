...
public class MBuscaCobros extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo",  "Monto", "Descripcion"};
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vcobro WHERE idcobros LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vcobro  WHERE idcobros LIKE '%" + filtro + "%' ORDER BY idcobros");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idcobros");
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE cobros SET estado ='Anulado' WHERE idcobros ='" + idModelo + "' ";
