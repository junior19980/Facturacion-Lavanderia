...
public class MBuscaServicioRealizado extends ConexionBD {
     ...
     */
    public MBuscaServicioRealizado() {
    }
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo",  "Fecha", "Nombre Funcionario"};
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vserviciorealizado WHERE idServicioRealizado LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vserviciorealizado  WHERE idServicioRealizado LIKE '%" + filtro + "%' ORDER BY idServicioRealizado");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE serviciosrealizados SET idestado ='2' WHERE idServicioRealizado ='" + idModelo + "' ";
