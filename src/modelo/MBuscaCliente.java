...
public class MBuscaCliente extends ConexionBD {
...
public MBuscaCliente() {
    ...
    public DefaultTableModel getTablaBusqueda() {
      ...
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        ...
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM Clientes WHERE idCliente LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM Clientes  WHERE idCliente LIKE '%" + filtro + "%' ORDER BY idCliente");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
