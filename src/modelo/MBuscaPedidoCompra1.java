...
public class MBuscaPedidoCompra1 extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
      ...
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        ...
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vpedidoorden WHERE idPedidoCompra LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vpedidoorden  WHERE idPedidoCompra LIKE '%" + filtro + "%' ORDER BY idPedidoCompra");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idPedidoCompra");
     ...
     public boolean Modificar(String idModelo, String Descrip){
        String q = "UPDATE detallespedidos SET estado ='Inactivo' WHERE idPedidoCompra ='" + idModelo + "' and idInsumo ='" + Descrip + "' ";
