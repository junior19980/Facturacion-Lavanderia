...
public class MBuscaPromocion extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo",  "Fecha Inicio", "Descripcion"};
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vpromocion WHERE idPromociones LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vpromocion  WHERE idPromociones LIKE '%" + filtro + "%' ORDER BY idPromociones");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idPromociones");
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE promociones SET estado ='Anulado' WHERE idPromociones ='" + idModelo + "' ";
