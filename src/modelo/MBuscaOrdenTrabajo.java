...
public class MBuscaOrdenTrabajo extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo",  "idInsumo", "Descripcion", "Cantidad", "Precio",  "Nombre", "Ruc"};
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vodenesserviciosrealizados WHERE idOrdenTrabajo LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vodenesserviciosrealizados  WHERE idOrdenTrabajo LIKE '%" + filtro + "%' ORDER BY idOrdenTrabajo");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
     ...
     public boolean Modificar(String idModelo, String idServicio){
        String q = "UPDATE detallesordenestrabajos SET Estado ='Anulado' WHERE idOrdenTrabajo ='" + idModelo +  "' and   idServicio ='" + idServicio +  "' ";
