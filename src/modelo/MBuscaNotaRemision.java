...
public class MBuscaNotaRemision extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
      ...
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        ...
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vnotaremision WHERE idNotaRemision LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vnotaremision  WHERE idNotaRemision LIKE '%" + filtro + "%' ORDER BY idNotaRemision");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idNotaRemision");
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE notasremisiones SET estado ='Anulado' WHERE idNotaRemision ='" + idModelo + "' ";
