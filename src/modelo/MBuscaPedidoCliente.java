...
public class MBuscaPedidoCliente extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
      ...
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        ...
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vpedidocliente WHERE idPedidoCliente LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vpedidocliente  WHERE idPedidoCliente LIKE '%" + filtro + "%' ORDER BY idPedidoCliente");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idPedidoCliente");
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE pedidosclientes SET estado ='Anulado' WHERE idPedidoCliente ='" + idModelo + "' ";
