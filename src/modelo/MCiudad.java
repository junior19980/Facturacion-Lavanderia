...
public class MCiudad extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo", "Ciudad"};
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM Ciudades  WHERE Ciudad LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM Ciudades  WHERE Ciudad LIKE '%" + filtro + "%' ORDER BY idCiudad");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idCiudad");
                data[i][1] = res.getString("Ciudad");
...
if( valida_datos(Cargo))
         {
            //Se arma la consulta
            String q = " INSERT INTO Ciudades ( idCiudad , Ciudad) "
    ...
    public boolean Actualizar(String id, String nombre) {
        String q = "UPDATE Ciudades SET Ciudad='" + nombre + "'  WHERE idCiudad ='" + id + "' ";
    ...
    public boolean Eliminar(String id) {
        boolean res = false;
        //se arma la consulta
        String q = " DELETE FROM Ciudades WHERE idCiudad='" + id + "' ";
        //se ejecuta la consulta
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement(q);
            pstm.execute();
    ...
    public void generar_codigo() {
        try {
            PreparedStatement pstm2 = this.getConnection().prepareStatement("select MAX(idCiudad) + 1 as id from Ciudades");
