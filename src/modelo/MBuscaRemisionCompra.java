...
public class MBuscaRemisionCompra extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo",  "Fecha", "Motivo"};
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM remisioncompra WHERE idNotaremision LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM remisioncompra  WHERE idNotaremision LIKE '%" + filtro + "%' ORDER BY idNotaremision");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idNotaremision");
                data[i][1] = res.getString("Fecha");
                data[i][2] = res.getString("Motivo");
                i++;
            }
            res.close();
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE remisionescompra SET Estado ='Anulado' WHERE idNotaremision ='" + idModelo + "' ";
