...
public class MBuscaCuentaCobrar extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo Factura",  "C.I-RUC", "Monto"};
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM cuentacobrar1 WHERE CI_Ruc LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM cuentacobrar1  WHERE idCuentaCobrar LIKE '%" + filtro + "%' ORDER BY idCuentaCobrar");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idCuentaCobrar");
                data[i][1] = res.getString("CI_Ruc");
                data[i][2] = res.getString("cobrar");
                i++;
            }
            res.close();
