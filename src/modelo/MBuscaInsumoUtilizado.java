...
public class MBuscaInsumoUtilizado extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
      ...
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        ...
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vinsumoutilizado WHERE idInsumoUtilizado LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vinsumoutilizado  WHERE idInsumoUtilizado LIKE '%" + filtro + "%' ORDER BY idInsumoUtilizado");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idInsumoUtilizado");
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE insumosutilizados SET estado ='Anulado' WHERE idInsumoUtilizado ='" + idModelo + "' ";
