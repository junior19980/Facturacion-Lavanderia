...
public class MBuscaFactura extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo",  "Fecha", "Observaciones"};
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vfacturaventa WHERE NroFacturaVentas LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vfacturaventa  WHERE NroFacturaVentas LIKE '%" + filtro + "%' ORDER BY NroFacturaVentas");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("NroFacturaVentas");
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE facturaventas SET estado ='Anulado' WHERE NroFacturaVentas ='" + idModelo + "' ";
        String q1 = "UPDATE cuentascobrar SET estado ='Anulado' WHERE NroFacturaVentas ='" + idModelo + "' ";
        String q2= "UPDATE libroiva SET estado ='Anulado' WHERE idFacturaVentas ='" + idModelo + "' ";
