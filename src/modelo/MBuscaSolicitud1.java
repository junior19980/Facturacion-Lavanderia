...
public class MBuscaSolicitud1 extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
      ...
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        ...
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vsolicitudordens WHERE idSolicitudCliente LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vsolicitudordens  WHERE idSolicitudCliente LIKE '%" + filtro + "%' ORDER BY idSolicitudCliente");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
     ...
     public boolean Modificar(String idModelo, String idServicio){
        String q = "UPDATE detallessolicitudesclientes SET Estado ='2' WHERE idSolicitudCliente ='" + idModelo +  "' and   idServicio ='" + idServicio +  "' ";
