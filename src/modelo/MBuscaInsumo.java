...
public class MBuscaInsumo extends ConexionBD {
...
public MBuscaInsumo() {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo",  "Nombre Insumo", "Precio"};
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vinsumo WHERE idInsumo LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vinsumo  WHERE idInsumo LIKE '%" + filtro + "%' ORDER BY idInsumo");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
    ...
    public void Reportes(String archivo) {
        try {
            PreparedStatement psta = this.getConnection().prepareStatement("select * FROM vinsumo order by idInsumo");
