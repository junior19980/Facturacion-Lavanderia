...
public class MBuscaContrato extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
      ...
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        ...
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vcontratos WHERE idContratos LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vcontratos  WHERE idContratos LIKE '%" + filtro + "%' ORDER BY idContratos");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idContratos");
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE contratos SET estado ='Anulado' WHERE idContratos ='" + idModelo + "' ";
