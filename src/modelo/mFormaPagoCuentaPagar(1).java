...
public class mFormaPagoCuentaPagar extends ConexionBD {
...
//Factura Ventas Tabla Principal
    ...
    public boolean RegistrarVenta( String Codigo,String Fecha, String idUsuarios,  String idCliente, String fdasf,String sldf,String kfd) {
        try {
	
			String q =  " insert into Pagos(idPago,fecha,Descripcion, idUsuario,monto,idCuentaPagar)"
			+ "values('" + Nro_Factura + "', '" + Fecha + "', '" + idUsuarios + "','" + idCliente +"', '" + sldf + "', '" + kfd + "')";
			PreparedStatement pstm = this.getConnection().prepareStatement(q);
			pstm.execute();
			pstm.close();
				String a=" UPDATE CuentasPagar SET contador =contador+'"+1+"' WHERE idCuentaPagar ='" + kfd + "'";
	...
	//Detalle 
	 public boolean RegistrarDetalle(String idaccesorio,String Canti,String monto) {
        try {
            String q2 =" insert into DetallesPagos(idPago,idFormatoPago,monto)"
...
//Detalle tabla secundario/
    public void generar_codigo() {
        try {
            PreparedStatement pstm2 = this.getConnection().prepareStatement("select MAX(idPago) + 1 as nro from Pagos");
            ...
            Logger.getLogger(mFormaPagoCuentaPagar.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
//Combo 1 Cliente
