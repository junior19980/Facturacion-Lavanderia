...
public class MTipoVehiculo extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo", "Tipo"};
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM TiposEquipos  WHERE Descripcion LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM TiposEquipos  WHERE Descripcion LIKE '%" + filtro + "%' ORDER BY idTipoEquipo");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idTipoEquipo");
...
if( valida_datos(Cargo))
         {
            //Se arma la consulta
            String q = " INSERT INTO TiposEquipos ( idTipoEquipo , Descripcion) "
    ...
    public boolean Actualizar(String id, String nombre) {
        String q = "UPDATE TiposEquipos SET Descripcion='" + nombre + "'  WHERE idTipoEquipo='" + id + "' ";
    ...
    public boolean Eliminar(String id) {
        boolean res = false;
        //se arma la consulta
        String q = " DELETE FROM TiposEquipos WHERE idTipoEquipo='" + id + "' ";
        //se ejecuta la consulta
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement(q);
            pstm.execute();
    ...
    public void generar_codigo() {
        try {
            PreparedStatement pstm2 = this.getConnection().prepareStatement("select MAX(idTipoEquipo) + 1 as id from TiposEquipos");
            ...
            Logger.getLogger(MTipoVehiculo.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
