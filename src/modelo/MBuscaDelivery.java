...
public class MBuscaDelivery extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
      ...
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        ...
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vdelivery WHERE idDelivery LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vdelivery  WHERE idDelivery LIKE '%" + filtro + "%' ORDER BY idDelivery");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idDelivery");
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE delivery SET estado ='Anulado' WHERE idDelivery ='" + idModelo + "' ";
