...
public class MCargo extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo", "Cargo"};
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM Cargos  WHERE Descripcion LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM Cargos  WHERE Descripcion LIKE '%" + filtro + "%' ORDER BY idCargo");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idCargo");
...
if( valida_datos(Cargo))
         {
            //Se arma la consulta
            String q = " INSERT INTO Cargos ( idCargo , Descripcion) "
    ...
    public boolean Actualizar(String id, String nombre) {
        String q = "UPDATE Cargos SET Descripcion='" + nombre + "'  WHERE idCargo ='" + id + "' ";
    ...
    public boolean Eliminar(String id) {
        boolean res = false;
        //se arma la consulta
        String q = " DELETE FROM Cargos WHERE idCargo='" + id + "' ";
        //se ejecuta la consulta
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement(q);
            pstm.execute();
    ...
    public void generar_codigo() {
        try {
            PreparedStatement pstm2 = this.getConnection().prepareStatement("select MAX(idCargo) + 1 as id from Cargos");
    ...
    public void Reportes(String archivo) {
        try {
            PreparedStatement psta = this.getConnection().prepareStatement("select * FROM Cargos order by idCargo");
