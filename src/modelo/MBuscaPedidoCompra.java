...
public class MBuscaPedidoCompra extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
      ...
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        ...
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM vpedidocompra WHERE NumPedidoCompra LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM vpedidocompra  WHERE NumPedidoCompra LIKE '%" + filtro + "%' ORDER BY NumPedidoCompra");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("NumPedidoCompra");
                data[i][1] = res.getString("Fecha");
                data[i][2] = res.getString("Observacion");
                i++;
            }
            res.close();
     ...
     public boolean Modificar(String idModelo){
        String q = "UPDATE pedidocompra SET estado ='Anulado' WHERE NumPedidoCompra ='" + idModelo + "' ";
