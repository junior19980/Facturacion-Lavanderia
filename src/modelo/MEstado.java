...
public class MEstado extends ConexionBD {
    ...
    public DefaultTableModel getTablaBusqueda() {
        ...
        int registros = 0;
        String[] columNames = {"Codigo", "Estado"};
      //obtenemos la cantidad de registros existentes en la tabla y se almacena en la variable "registros"
        //para formar la matriz de datos
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT count(*) as total FROM estados  WHERE estado LIKE '%" + filtro + "%' ");
        ...
        try {
            ...
            PreparedStatement pstm = this.getConnection().prepareStatement("SELECT * FROM estados  WHERE estado LIKE '%" + filtro + "%' ORDER BY idEstado");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                data[i][0] = res.getString("idEstado");
...
if( valida_datos(Cargo))
         {
            //Se arma la consulta
            String q = " INSERT INTO estados ( idEstado , estado) "
    ...
    public boolean Actualizar(String id, String nombre) {
        String q = "UPDATE estados SET estado='" + nombre + "'  WHERE idEstado ='" + id + "' ";
    ...
    public boolean Eliminar(String id) {
        boolean res = false;
        //se arma la consulta
        String q = " DELETE FROM estados WHERE idEstado='" + id + "' ";
        //se ejecuta la consulta
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement(q);
            pstm.execute();
    ...
    public void generar_codigo() {
        try {
            PreparedStatement pstm2 = this.getConnection().prepareStatement("select MAX(idEstado) + 1 as id from estados");
    ...
    public void Reportes(String archivo) {
        try {
            PreparedStatement psta = this.getConnection().prepareStatement("select * FROM estados order by idEstado");
