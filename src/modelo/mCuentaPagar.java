...
public class mCuentaPagar extends ConexionBD {
    public ResultSet rs;
       public ResultSet r;
    vResepcion Vista;
    public int totales3 = 0;
    public String NroFactura = "",Nrocuenta="", Monto="";
    
//Factura Ventas Tabla Principal

    public boolean RegistrarVenta( String Codigo,String Fecha, String idUsuarios, String idEquipo, String idCliente, String hola) {
        try {
            String q =    " insert into CuentasPagar(idCuentaPagar,idFacturaCompra,idUsuario,fechaVencimiento,monto, cantidad)"
                    + "values('" + Nrocuenta + "', '" + NroFactura + "', '" + idUsuarios + "', '" + idEquipo + "','" + Monto +"','" + hola +"')";
...
//Detalle tabla secundario/
     ...
     public void genera() {
        try {
            PreparedStatement pstm2 = this.getConnection().prepareStatement("select MAX(idCuentaPagar) + 1 as nro from CuentasPagar");
            ...
            Logger.getLogger(mCuentaPagar.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
      public void generar_codigo() {
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("select MAX(idFacturaCompra)  as nro from FacturasCompras");
      ...
      public void generar_Monto() {
        try {
            PreparedStatement pstm = this.getConnection().prepareStatement("select MAX(Total)  as nro from FacturasCompras");
            rs = pstm.executeQuery();
            rs.next();
           
                Monto= rs.getString("nro");
